...
Generator script for mu-tuning.

Usage examples:

1) Linear beta axis, coarse mu scan for attractive Hubbard (U=-6) at nt=0.8, 6x6, tp=tpp=0, nflux=0, periodic bc:

    python3 gen_beta_mu_scan.py \
        --beta_axis_mode linear --beta_min 2 --beta_max 10 --n_beta 5 \
        --dt_tgt 0.05 --L_min 40 --dt_policy closest \
        --generator_path /home/users/phoenixm/dqmc-dev/util/gen_1band_unified_hub.py \
        --output_path /scratch/users/phoenixm/beta_mu_scan_Um6_nt08 \
        --output_prefix Um6_nt08_ \
        --geometry square --Nx 6 --Ny 6 --U -6 --hs_channel auto --tp 0 --tpp 0 --nflux 0 --bc 1 \
        --nt 0.8 --mu_list "-4,-2,0,2,4"

2) Beta list, coarse mu scan for nt=0.6:

    python3 gen_beta_mu_scan.py \
        --beta_list "10,8,6,4,2" \
        --dt_tgt 0.05 --L_min 40 --dt_policy closest \
        --generator_path /home/users/phoenixm/dqmc-dev/util/gen_1band_unified_hub.py \
        --output_path /scratch/users/phoenixm/beta_mu_scan_Um6_nt06 \
        --output_prefix Um6_nt06_ \
        --geometry square --Nx 6 --Ny 6 --U -6 --hs_channel auto --tp 0 --tpp 0 --nflux 0 --bc 1 \
        --nt 0.6 --mu_list "-4,-2,0,2,4"

3) Dry-run example (print commands only, do not execute):

    python3 gen_beta_mu_scan.py \
        --beta_axis_mode linear --beta_min 2 --beta_max 10 --n_beta 3 \
        --dt_tgt 0.05 --L_min 40 --dt_policy closest \
        --generator_path /home/users/phoenixm/dqmc-dev/util/gen_1band_unified_hub.py \
        --output_path /scratch/users/phoenixm/beta_mu_scan_dryrun \
        --output_prefix dryrun_ \
        --geometry square --Nx 6 --Ny 6 --U -6 --hs_channel auto --tp 0 --tpp 0 --nflux 0 --bc 1 \
        --nt 0.7 --mu_list "-3,0,3" --dry_run

4) Example with nonzero Zeeman field and twist phases:

    python3 gen_beta_mu_scan.py \
        --beta_axis_mode linear --beta_min 2 --beta_max 8 --n_beta 4 \
        --dt_tgt 0.05 --L_min 40 --dt_policy closest \
        --generator_path /home/users/phoenixm/dqmc-dev/util/gen_1band_unified_hub.py \
        --output_path /scratch/users/phoenixm/beta_mu_scan_twist \
        --output_prefix twist_ \
        --geometry square --Nx 6 --Ny 6 --U -6 --hs_channel auto --tp 0 --tpp 0 --nflux 0 --bc 1 \
        --nt 0.75 --mu_list "-2,0,2" --h 0.2 --twistx 0.1 --twisty 0.05

5)
python3 /home/users/phoenixm/scripts/gen_beta_mu_scan.py \
  --beta_list "0.125,0.25,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5" \
  --dt_tgt 0.05 --L_min 40 \
  --n_matmul 5 --dt_policy closest \
  --generator_path /home/users/phoenixm/dqmc-dev/util/gen_1band_unified_hub.py \
  --output_path  /scratch/users/phoenixm/dqmc_runs/ \
  --geometry square --Nx 6 --Ny 6 --U -6 --tp 0 --tpp 0 --nflux 0 --bc 1 --nt 0.8 \
  --mu_list="-3,-2.75,-2.5,-2.25,-2,-1.75,-1.5,-1.25,-1,-0.75,-0.5,-0.25,0" \
  --hs_channel density \
  --Nfiles 100 --h=0.0 --n_delay=16 --n_sweep_warm=20000 --n_sweep_meas=400000 \
  --period-eqlt=5 --period-uneqlt=0 --trans_sym=1 --checkpoint_every=10000 \
  --meas_bond_corr=0 --meas_energy_corr=0 --meas_nematic_corr=0 --meas_thermal=0 \
  --meas_2bond_corr=0 --meas_chiral=0
"""
...
def parse_args():
    ...
    p.add_argument("--output_npath_suffix", default=None, 
                   help="Manually setting the suffix of the {output_path}/n*_{output_npath_suffix}/ directory.")
    ...
    # ---- physics ----
    p.add_argument("--nt", type=float, required=True, help="Target filling.")
    ...
    p.add_argument("--mu_list", type=str, required=True,
                   help="Comma-separated chemical potential list for mu-tuning, e.g. '-4,-2,0'.")
...
def build_mu_axis(args):
    mus = [float(x) for x in args.mu_list.split(",") if x.strip()]
...
def main():
    ...
    for beta in sorted(betas):
        ...
        for mu in sorted(mus):
            ...
            U = args.U
            nt = args.nt
            sub = output_path / f"n{nt:.6g}" / f"T{T:.6g}_beta{beta:.6g}_U{U:.6g}" / f"mu{mu:.6g}"
            if args.output_npath_suffix is not None:
                suffix = str(args.output_npath_suffix)
                sub = output_path / f"n{nt:.6g}_{suffix}" / f"T{T:.6g}_beta{beta:.6g}_U{U:.6g}" / f"mu{mu:.6g}"
            ...
            try:
                ...
                if not args.dry_run:
                    ...
                    for f in Path(".").glob(f"{prefix}*.h5"):
                        ...
                        pass
                summary_rows.append((T, beta, L, dt, mu, str(sub), prefix))
            finally:
                os.chdir(cwd)

    # write summary (append mode)
    sum_path = output_path / f"n{args.nt:.6g}" / "summary.tsv"
    if args.output_npath_suffix is not None:
        suffix = str(args.output_npath_suffix)
        sum_path = output_path / f"n{nt:.6g}_{suffix}" / "summary.tsv"
    sum_path.parent.mkdir(parents=True, exist_ok=True)

    already_has_header = sum_path.exists() and sum_path.stat().st_size > 0
    with open(sum_path, "a") as f:
        if not already_has_header:
            ...
            f.write("T\tbeta\tL\tdt\tmu\tdir\tprefix\n")
        for T, beta, L, dt, mu, sub, prefix in sorted(summary_rows, key=lambda r: (r[0], r[4])):
            f.write(f"{T:.12g}\t{beta:.12g}\t{L}\t{dt:.12g}\t{mu:.12g}\t{sub}\t{prefix}\n")
    print(f"[OK] updated {sum_path}")
    print("[DONE]")
